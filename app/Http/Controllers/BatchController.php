...
class BatchController extends Controller
{
    ...
    public function approveBatchListOfficer(Request $request)
    {
        if (session::get('user_id')) {
            $batch_data = BatchMaster::get(['id', 'name']);
            $approveserialdata = DB::select("SELECT *, t1.id as serial_id FROM `serial_no_batches` as t1 Left JOIN product_batch_masters as t2 ON t2.id = t1.product_batch_id WHERE t1.batch_id = 1 and t1.officer_status = 1");
            $sidebar_btn = ['UI Elements', 'Item List', 'Inventory'];
